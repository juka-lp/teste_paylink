...
class EletronicDocument(models.Model):
    ...
    lancar_dif = fields.Many2one('product.product', string='Lançar Dif. em:')
    ...
    def get_ICMSTot(self, nfe):
        ...
        return dict(
            ...
            valor_despesas=get(ICMSTot, 'vOutro'),
            valor_fcpst=get(ICMSTot, 'vFCPST'),
        )
    ...
    def create_invoice_eletronic_item(self, item, company_id, partner_id,
        ...
        ### JUNIOR ###
        product_prod_anvisa = None
        product_pmc = 0
        if hasattr(item.prod, 'med'):
            if hasattr(item.prod.med, 'cProdANVISA'):
                product_prod_anvisa = str(item.prod.med.cProdANVISA)
            
            if hasattr(item.prod.med, 'vPMC'):
                product_pmc = item.prod.med.vPMC
        
        if product:
            product_po_uom = product.uom_po_id.id
        else:
            product_po_uom = uom_id

        product_codigo_anp = 0
        product_descricao_anp = None
        if hasattr(item.prod, 'comb'):
            product_codigo_anp = int(item.prod.comb.cProdANP)
            product_descricao_anp = str(item.prod.comb.descANP)
        ##############
        
        invoice_eletronic_Item = {
            'product_id': product_id, 'uom_id': product_po_uom, # JUNIOR
            ...
            'product_prod_anvisa': product_prod_anvisa, 'product_pmc': product_pmc, # JUNIOR
            'product_codigo_anp': product_codigo_anp, 'product_descricao_anp': product_descricao_anp # JUNIOR
        }
        ...
        for cst in csts:
            ...
            if tag_icms:
                ...
                vals = {
                    ...
                    'icms_bc_st_retido':get(icms, '%s%s.vBCSTRet' % (tag_icms, cst)),
                    'icms_st_retido': get(icms, '%s%s.vICMSSTRet' % (tag_icms, cst)),
                }
    ...
    def _create_product(self, company_id, supplier, nfe_item, uom_id=False):
        ...
        ######## JUNIOR ########
        if hasattr(nfe_item, 'comb'):
            product.update({
                'is_anp': True,
                'codigo_anp': get(nfe_item.comb, 'cProdANP', int),
                'descricao_anp': get(nfe_item.comb, 'descANP', str),
            })
        ########################
    ...
    def prepare_account_invoice_line_vals(self, item):
        if item.product_id:
            ...
            product = item.product_id.with_company(self.company_id.id)
        ...
        if item.cfop not in ['5910', '6910']:
            vals = {
                'product_id': item.product_id.id,
                'product_uom_id': item.uom_id.id,
                'name': item.name if item.name else item.product_xprod,
                'quantity': item.quantidade,
                'price_unit': (item.valor_bruto-item.icms_valor_desonerado)/item.quantidade, # JUNIOR
                'discount': float_round((item.desconto/(item.valor_bruto-item.icms_valor_desonerado))*100, precision_digits=5) if item.desconto else 0.0, # JUNIOR
        ...
        #         'name': item.name if item.name else item.product_xprod,
        #         'quantity': item.quantidade,
        #         'price_unit': (item.valor_bruto-item.icms_st_valor)/item.quantidade, # JUNIOR
        #         'discount': 0.0, # JUNIOR
        #         'account_id': account_id.id,
        #     }
        else:
            vals = {
                ...
                'name': item.name if item.name else item.product_xprod,
                'quantity': 1.0,
                'price_unit': -item.icms_st_valor, # JUNIOR
                'discount': 0.0, # JUNIOR
                'account_id': account_id.id,
            }
        ###############
    ...
    def _prepare_account_invoice_vals(self):
        ...
        partner = self.partner_id.with_company(self.company_id.id)
    ...
    def generate_account_move(self):
        ...
        ### JUNIOR ###
        if self.valor_fcpst:
            product = self.env.ref("l10n_br_nfe_import.product_product_tax_fcpst")
            items.append((0, 0, self.prepare_extra_line_items(product, self.valor_fcpst)))

        if self.lancar_dif:
            valor_dif = self.fatura_liquido - self.valor_final
            if valor_dif > 0:
                items.append((0, 0, self.prepare_extra_line_items(self.lancar_dif, valor_dif)))
        ##############
...
class EletronicDocumentLine(models.Model):
    ...
    product_prod_anvisa = fields.Char('Cód. Produto ANVISA') # JUNIOR
    product_pmc = fields.Float('Preço Máximo Consumidor (PMC)') # JUNIOR
    product_codigo_anp = fields.Integer(string='Código ANP (XML)') # JUNIOR
    product_descricao_anp = fields.Char(string='Descrição ANP (XML)') # JUNIOR